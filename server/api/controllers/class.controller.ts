...
export class ClassController {
  ...
  async classFileUpload(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      let img_data:any = fs.readFileSync(req["files"]["image"][0]["path"]);
      let upload_file_data: any = fs.readFileSync(
        req["files"]["upload_file"][0]["path"]
      );
      let imgdata: String = Buffer.from(img_data, 'binary').toString('base64');
      let imgdata1: String = Buffer.from(upload_file_data, 'binary').toString('base64');
      let file_obj = {
