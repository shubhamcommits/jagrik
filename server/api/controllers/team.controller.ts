...
export class TeamController {
   ...
   * @param next
   */
  async rejectTeamTask(req: Request, res: Response, next: NextFunction) {
    try {
      // Fetch the authorization header from the request
  ...
  /**
   * This function is responsible for getting team profile
   * @param req
   * @param res
   * @param next
   */
  async teamProfile(req: Request, res: Response, next: NextFunction) {
    try {
      // Fetch the authorization header from the request
      ...
      // Call the service function to get all the classes
      await teamService
        .teamProfile(authorization, teamId)
        .then((response) => {
          return res.status(200).json({
            message: 'data fetched successfully!',
            response: response,
